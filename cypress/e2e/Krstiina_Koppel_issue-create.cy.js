...
describe('Issue create', () => {
  ...
  it.only('Should create an issue and validate it successfully', () => {
    ...
    cy.get('[data-testid="modal:issue-create"]').within(() => {
      // Type value to description input field
      cy.get('.ql-editor').type('Random data information');
      cy.get('.ql-editor').should('have.text', 'Random data information');

      // Type value to title input field
      // Order of filling in the fields is first description, then title on purpose
      // Otherwise filling title first sometimes doesn't work due to web page implementation
      cy.get('input[name="title"]').type('Moon');
      cy.get('input[name="title"]').should('have.value', 'Moon');
